...
/**
 * ImageUpload component for Cloudinary integration
 * Props:
 *   value      — current image object { imageUrl, thumbnailUrl, publicId } or string
 *   onChange   — (imageData: { imageUrl, thumbnailUrl, publicId }) => void
 */
const ImageUpload = ({ value, onChange }) => {
    ...
    const [urlInput, setUrlInput] = useState(typeof value === 'string' ? value : value?.imageUrl || '');
    const [error, setError] = useState('');

    // Helper to get image URL from value (backward compatible)
    const getImageUrl = () => {
        if (!value) return '';
        if (typeof value === 'string') return value;
        return value.imageUrl || value.thumbnailUrl || '';
    };
    ...
    const handleUrlApply = () => {
        if (urlInput) {
            // For URL mode, just store as string (backward compatible)
            onChange(urlInput);
            setError('');
        }
    };

    const clear = () => { onChange(null); setUrlInput(''); };
        ...
        <div className="space-y-3">
            ...
            {mode === 'upload' ? (
                ...
                >
                    ...
                    {uploading ? (
                        <><Loader2 className="animate-spin text-blue-500" size={28} /><p className="text-sm text-gray-500">Uploading to Cloudinary…</p></>
                    ) : (
                        <>
            ...
            {imageUrl && (
                <div className="relative rounded-xl overflow-hidden border border-gray-100 shadow-sm">
                    <img src={imageUrl} alt="Preview" className="w-full h-40 object-cover" onError={e => e.target.src = 'https://via.placeholder.com/400x160?text=Invalid+Image'} />
                    ...
                    <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/40 to-transparent px-3 py-1.5">
                        <p className="text-white text-[10px] truncate">{imageUrl}</p>
                    </div>
                </div>
            )}
        </div>
